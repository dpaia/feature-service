...
/**
 * Event deduplication service with event store support
 ...
 * Stores full event metadata for replay functionality
 */
@Service
public class EventDeduplicationService {
    ...
    public EventDeduplicationService(EventStoreRepository eventStoreRepository, ObjectMapper objectMapper) {
     ...
     * Stores minimal metadata for simple operations
     *
    ...
    public <T> T executeIdempotent(String eventId, EventType eventType, Supplier<T> processor) {
        return executeIdempotentWithMetadata(eventId, eventType, null, null, null, null, processor);
    }

    /**
     * Execute idempotent operation with full event metadata storage
     * This method stores complete event information for replay support
     *
     * @param eventId the unique event identifier
     * @param eventType the event type (API or EVENT)
     * @param operationType the operation type (CREATED, UPDATED, DELETED)
     * @param featureId the feature database ID
     * @param featureCode the feature business code
     * @param eventPayload the full event payload as JSON
     * @param processor the processing logic to execute if event is not duplicate
     * @param <T> the type of result returned by the processor
     * @return the result of the processor if event was processed, or stored result if it was a duplicate
     */
    @Transactional
    public <T> T executeIdempotentWithMetadata(
            String eventId,
            EventType eventType,
        ...
        if (eventPayload != null) {
            try {
                ...
                logger.warn("Failed to serialize event payload for eventId: {}", eventId, e);
                payloadJson = eventPayload.toString();
            }
        }

        // Use unified database-first approach for both API and EVENT types
        return executeDatabaseFirst(
                eventId, eventType, operationType, featureId, featureCode, payloadJson, now, processor, now, expiresAt);
    }
    ...
    private <T> T executeDatabaseFirst(
        ...
        int inserted = eventStoreRepository.insertIfNotExists(
                eventId,
                eventType.name(),
                operationType != null ? operationType : "UNKNOWN",
                featureId,
                featureCode,
        ...
        if (inserted > 0) {
            ...
            try {
                ...
                updateEventStoreResult(eventId, eventType.name(), resultData);
            ...
            String storedResult = eventStoreRepository.getResultData(eventId, eventType.name(), now);
     ...
     */
    private void updateEventStoreResult(String eventId, String eventType, String resultData) {
        try {
            // Use a simple update query to replace the result_data
            int updated = eventStoreRepository.updateResultData(eventId, eventType, resultData);
            if (updated == 0) {
    ...
    /**
     * Clean up expired events from event store
    ...
    public int cleanupExpiredEvents() {
        int deletedCount = eventStoreRepository.deleteExpiredEvents(LocalDateTime.now());
        if (deletedCount > 0) {
            logger.info("Cleaned up {} expired events from event store", deletedCount);
        }
        return deletedCount;
    }
}
