...
public class FeatureService {
    ...
    public String createFeature(CreateFeatureCommand cmd) {
        // Pre-generate feature code for metadata
        ...
        // Use EventDeduplicationService for API-level idempotency with result storage and full metadata
        return eventDeduplicationService.executeIdempotentWithMetadata(
                cmd.eventId(),
                EventType.API,
                "CREATED",
                null, // featureId will be set after creation
                code, // featureCode is now known
                cmd, // store the command as event payload
                () -> {
                    // Create new feature with pre-generated code
                    Release release =
    ...
    public void updateFeature(UpdateFeatureCommand cmd) {
        // Get feature first to have ID for metadata
        Feature existingFeature = featureRepository.findByCode(cmd.code()).orElseThrow();

        // Use EventDeduplicationService for API-level idempotency with full metadata
        eventDeduplicationService.executeIdempotentWithMetadata(
                cmd.eventId(),
                EventType.API,
                "UPDATED",
    ...
    public void deleteFeature(DeleteFeatureCommand cmd) {
        // Try to get feature for metadata, but don't fail if it doesn't exist (idempotent case)
        // If feature doesn't exist, it might be a duplicate delete request
        Optional<Feature> optionalFeature = featureRepository.findByCode(cmd.code());
        Long featureId = optionalFeature.map(Feature::getId).orElse(null);
        ...
        eventDeduplicationService.executeIdempotentWithMetadata(
                ...
                cmd, // store the command as event payload
                () -> {
                    // Get feature before deletion - this will throw if feature doesn't exist
                    // which is correct for the first delete request
                    Feature featureToDelete =
                            featureRepository.findByCode(cmd.code()).orElseThrow();
                    ...
                    eventPublisher.publishFeatureDeletedEvent(
                            cmd.eventId(), featureToDelete, cmd.deletedBy(), Instant.now());
