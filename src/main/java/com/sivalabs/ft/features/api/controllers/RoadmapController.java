...
public class RoadmapController {
    ...
    public ResponseEntity<RoadmapResponseDto> getRoadmap(
        ...
        try {
            ...
            RoadmapResponseDto response = roadmapService.getRoadmap(filter);
    ...
    public ResponseEntity<MultiProductRoadmapResponseDto> getMultiProductRoadmap(
        ...
        try {
            ...
            MultiProductRoadmapResponseDto response =
                    roadmapService.getGroupedMultiProductRoadmap(productCodes, filter);
            return ResponseEntity.ok(response);
        } catch (IllegalArgumentException e) {
            log.warn("Invalid request parameters: {}", e.getMessage());
            return ResponseEntity.badRequest().build();
    ...
    public ResponseEntity<ByOwnerRoadmapResponseDto> getRoadmapByOwner(
        ...
        try {
            ...
            ByOwnerRoadmapResponseDto response = roadmapService.getStructuredRoadmapByOwner(owner, filter);
            return ResponseEntity.ok(response);
        } catch (IllegalArgumentException e) {
            log.warn("Invalid request parameters: {}", e.getMessage());
            return ResponseEntity.badRequest().build();
    ...
    public ResponseEntity<byte[]> exportRoadmap(
        ...
        try {
            ...
            if (owner != null) {
                roadmapData = roadmapService.getRoadmapByOwner(owner, filter);
            } else if (productCodes != null && !productCodes.isEmpty()) {
                roadmapData = roadmapService.getMultiProductRoadmap(productCodes, filter);
            } else {
                roadmapData = roadmapService.getRoadmap(filter);
            }
