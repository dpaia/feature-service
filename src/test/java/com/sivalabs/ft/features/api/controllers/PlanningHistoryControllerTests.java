...
class PlanningHistoryControllerTests extends AbstractIT {
    ...
    void shouldGetPlanningHistoryWithDefaultPagination() throws Exception {
        ...
        assertThat((String) firstHistory.get("changedBy")).isEqualTo("testuser");
    ...
    void shouldGetPlanningHistoryFilteredByEntityType() throws Exception {
        ...
        content.stream().forEach(history -> {
            ...
            assertThat((String) history.get("entityCode"))
                    .isIn(
                            "IDEA-110",
                            "IDEA-109",
                            "IDEA-108",
    ...
    void shouldGetFeatureHistoryByCode() throws Exception {
        ...
        assertThat((String) updatedHistory.get("fieldName")).isIn("title", "description", "assignedTo", "status");
    ...
    void shouldGetPlanningHistorySortedByChangedAtDescending() throws Exception {
        var result = mvc.get()
                .uri("/api/planning-history?entityType=FEATURE&changeType=CREATED&page=0&size=20&sort=changedAt,desc")
                .exchange();
        ...
        // Verify all records are sorted in descending order
        for (int i = 0; i < content.size() - 1; i++) {
            String current = (String) content.get(i).get("changedAt");
            String next = (String) content.get(i + 1).get("changedAt");
            Instant currentInstant = Instant.parse(current);
            Instant nextInstant = Instant.parse(next);
            assertThat(currentInstant).isAfterOrEqualTo(nextInstant);
        }
    }
    ...
    void shouldHandleDateRangeFilter() throws Exception {
        ...
        for (Map<String, Object> history : content) {
            ...
            assertThat(history.get("entityType")).isIn("FEATURE", "RELEASE");
            assertThat(((Number) history.get("entityId")).longValue()).isGreaterThan(0L);
            assertThat((String) history.get("entityCode")).isEqualTo("IDEA-109");
            assertThat(history.get("changeType"))
                    .isIn("CREATED", "UPDATED", "DELETED", "STATUS_CHANGED", "ASSIGNED", "MOVED");
            assertThat((String) history.get("changedBy")).isEqualTo("testuser");

            String changedAt = (String) history.get("changedAt");
            assertThat(changedAt).matches("\\d{4}-\\d{2}-\\d{2}T.*");

            // Parse changedAt to Instant and verify it's within the date range
            ...
            assertThat(changedAtInstant).isAfterOrEqualTo(dateFrom);
            assertThat(changedAtInstant).isBeforeOrEqualTo(dateTo);
    ...
    void shouldHandleChangedByFilter() throws Exception {
        ...
        for (Map<String, Object> history : content) {
            ...
            assertThat(history.get("entityType")).isIn("FEATURE", "RELEASE");
            assertThat(((Number) history.get("entityId")).longValue()).isEqualTo(feature.id());
            assertThat((String) history.get("entityCode")).isEqualTo(feature.code());
            assertThat(history.get("changeType"))
                    .isIn("CREATED", "UPDATED", "DELETED", "STATUS_CHANGED", "ASSIGNED", "MOVED");
            assertThat((String) history.get("changedBy")).isEqualTo("testuser");
            assertThat((String) history.get("changedAt")).matches("\\d{4}-\\d{2}-\\d{2}T.*");
    ...
    @Test
    void shouldReturnAuthErrorWhenAccessingPlanningHistoryWithoutAuthentication() {
        var result = mvc.get().uri("/api/planning-history").exchange();
    ...
    @Test
    void shouldReturnAuthErrorWhenAccessingPlanningHistoryWithPaginationWithoutAuthentication() {
        var result = mvc.get()
                .uri("/api/planning-history?page=0&size=5&sort=changedAt,asc")
                .exchange();
    ...
    @Test
    void shouldReturnAuthErrorWhenAccessingFeatureHistoryByCodeWithoutAuthentication() {
        var result =
    ...
    @Test
    void shouldReturnAuthErrorWhenAccessingReleaseHistoryByCodeWithoutAuthentication() {
        var result =
    ...
    @WithMockOAuth2User(
            ...
            roles = {"GUEST"})
    void shouldReturnAuthErrorWhenAccessingPlanningHistoryWithInsufficientPermissions() {
        var result = mvc.get().uri("/api/planning-history").exchange();
    ...
    @WithMockOAuth2User(
            username = "restricteduser",
            roles = {"GUEST"})
    void shouldReturnAuthErrorWhenAccessingPlanningHistoryWithPaginationWithInsufficientPermissions() {
        var result = mvc.get()
                .uri("/api/planning-history?page=0&size=5&sort=changedAt,asc")
                .exchange();
    ...
    @WithMockOAuth2User(
            username = "restricteduser",
            roles = {"GUEST"})
    void shouldReturnAuthErrorWhenAccessingFeatureHistoryByCodeWithInsufficientPermissions() {
        var result =
                mvc.get().uri("/api/features/{code}/history", "TEST-FEATURE").exchange();
    ...
    @WithMockOAuth2User(
            username = "restricteduser",
            roles = {"GUEST"})
    void shouldReturnAuthErrorWhenAccessingReleaseHistoryByCodeWithInsufficientPermissions() {
        var result =
                mvc.get().uri("/api/releases/{code}/history", "TEST-RELEASE").exchange();
