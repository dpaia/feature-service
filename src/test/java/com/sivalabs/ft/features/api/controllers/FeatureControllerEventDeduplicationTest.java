...
public class FeatureControllerEventDeduplicationTest extends AbstractIT {
    ...
    void testEventDeduplicationCleanup() throws Exception {
        ...
        // Insert expired event directly into database with all required fields
        jdbcTemplate.update(
                "INSERT INTO event_store (event_id, event_type, operation_type, event_payload, event_timestamp, processed_at, expires_at, result_data) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                expiredEventId,
                "EVENT",
                "CREATED",
                "{}",
        ...
        int deletedCount = jdbcTemplate.update("DELETE FROM event_store WHERE expires_at < ?", LocalDateTime.now());
    ...
    private int getProcessedEventsCount() {
        try {
            return jdbcTemplate.queryForObject("SELECT COUNT(*) FROM event_store", Integer.class);
        } catch (Exception e) {
            return 0;
        }
    }
    ...
    private int getProcessedEventsCountByType(String eventType) {
        try {
            return jdbcTemplate.queryForObject(
                    "SELECT COUNT(*) FROM event_store WHERE event_type = ?", Integer.class, eventType);
        } catch (Exception e) {
            return 0;
        }
    }
    ...
    private int getProcessedEventsCountByEventId(String eventId) {
        try {
            Integer count = jdbcTemplate.queryForObject(
                    "SELECT COUNT(*) FROM event_store WHERE event_id = ?", Integer.class, eventId);
            return count != null ? count : 0;
        } catch (Exception e) {
            return 0;
        }
