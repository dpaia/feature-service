...
/**
 * Test configuration that creates required Kafka topics during Spring Boot context startup.
 * This ensures topics are available when ApplicationLifecycleListener performs startup verification.
 * Also ensures KafkaTemplate and AdminClient beans are available for ApplicationLifecycleListener's @ConditionalOnBean.
 *
...
public class TestKafkaTopicConfiguration {
    ...
    /**
     * Create a test KafkaTemplate to satisfy ApplicationLifecycleListener's @ConditionalOnBean requirement.
     * This ensures ApplicationLifecycleListener bean is created in test environments.
     * NOT marked as @Lazy to ensure it's available during @ConditionalOnBean evaluation.
     */
    @Bean
    @Primary
    ...
    /**
     * Create a test AdminClient to satisfy ApplicationLifecycleListener's @ConditionalOnBean requirement.
     * This ensures both KafkaTemplate and AdminClient are available in test context.
     * NOT marked as @Lazy to ensure it's available during @ConditionalOnBean evaluation.
     */
    @Bean
    @Primary
     ...
     * Uses highest priority (@Order(Integer.MIN_VALUE)) to run before ApplicationLifecycleListener.
     */
     ...
     * This bypasses the @ConditionalOnBean issue where Spring can't find the required beans
